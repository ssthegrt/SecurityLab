...
string encrypt(string publicKey, string Message)
{
	...
	cout<<"n:  "<<token_n << endl <<" e:"<<token_e.c_str()<<" h"<<endl;
	mpz_t e,n;
...
string ReEncrypt(string Cipher, string privateKey, string publicKey)
{
	cout << "Currently The CipherText is : " ;
	cout << Cipher << endl;
	string s = decrypt(privateKey, Cipher);
	string ss = encrypt(publicKey,s);
	cout << "After Re-encryption, the string obtained is : " << ss << endl;
	return ss;
}
